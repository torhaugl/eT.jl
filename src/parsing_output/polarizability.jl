const polarizability_regex::Regex =
    r"<< mu_[xyz], mu_[xyz] >>\(-?\d+.\d+[eE][-+]?\d+\): +(-?\d+.\d+)"
export get_polarizability

function get_polarizability(out::OutputFile)
    polarizability_vector = parse.(Float64, [x.captures[1] for x in eachmatch(polarizability_regex, out.contents["eT"])])
